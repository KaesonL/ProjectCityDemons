...
float Sobel_Normal_Horizontal(sampler2D texmap, vec2 texcoord)
{
	...
	if (edgeFactor < 4.0) //Number depends on the scale of the scene
	{
		return 1.0;
	}
	else
...
float Sobel_Depth_Horizontal(sampler2D texmap, vec2 texcoord)
{
	...
	if (sum < 0.03) //THreshold number depends on the scale of the scene
	{
		return 1.0;
	}
	else
...
float Sobel_Depth_Vertical(sampler2D texmap, vec2 texcoord)
{
	...
	if (sum < 0.03) //Number depends on the scale of the scene
	{
		return 1.0;
	}
	else
