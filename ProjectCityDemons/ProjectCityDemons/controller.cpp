...
#pragma endregion
	...
	bool XBoxInput::GetButton(int _index, Button _buttons)
	{
		if (!m_connected[_index]) return false;
		//button trigger
		else if (_buttons == Button::LT) {
			if (m_triggers[_index].first > m_triggerButtonSens[_index])//left
				return true;
			else
				return false;
		}
		else if (_buttons == Button::RT) {
			if (m_triggers[_index].second > m_triggerButtonSens[_index])//right
				return true;
			else
				return false;
		}
